from gr_utilities._paramtreecfg import cfg
...
def makeChild(chType, cfgDict):
    ...
    grp = Parameter.create(name=f'{chType.title()}', type='group', children=metaChildren + [param] + optsChildren)
...
def makeAllParamTypes():
    ...
    params = Parameter.create(name='Parameters', type='group', children=children)

    # Slider needs minor tweak
    sliderGrp = params.child('Slider')
